...
@st.cache_resource
def load_artefacts():
    # Load CatBoost natively
    model = CatBoostClassifier()
    model.load_model(str(BASE_DIR / "models" / "catboost_model.cbm"))

    # Load only passthrough features — small, safe
    ...
    # Rebuild scaler — fit on dummy transformed output so dimensions match
    sc = StandardScaler(with_mean=False)
    sc.fit(ct.transform(dummy))
